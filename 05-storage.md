...
# [05-storage：ストレージ](#storage) {#storage}

## [はじめに](#introduction) {#introduction}

[04-network](../04-network/) では、コンテナがどうやって通信するかを学びました

bridge、veth ペア、ポートマッピングにより、コンテナは外部や他のコンテナとネットワーク通信ができます

しかし、コンテナにはもう1つ重要な課題があります

[03-image](../03-image/) で学んだように、コンテナのファイルシステムは<strong>コンテナレイヤ</strong>（書き込み可能レイヤ）に書き込まれます
...
1. [コンテナのファイルシステム](#container-filesystem)
2. [なぜデータの永続化が必要か](#why-data-persistence)
3. [ボリューム](#volume)
4. [バインドマウント](#bind-mount)
5. [tmpfs マウント](#tmpfs-mount)
6. [ボリューム・バインドマウント・tmpfs の比較](#storage-options-comparison)
7. [ストレージドライバ](#storage-driver)
...
## [コンテナのファイルシステム](#container-filesystem) {#container-filesystem}

[03-image](../03-image/) で学んだように、コンテナのファイルシステムは以下の構造になっています
...
### [コンテナレイヤの特徴](#container-layer-characteristics) {#container-layer-characteristics}
...
## [なぜデータの永続化が必要か](#why-data-persistence) {#why-data-persistence}
...
### [ボリュームの作成と使用](#volume-creation-and-use) {#volume-creation-and-use}
...
### [ボリュームの保存場所](#volume-storage-location) {#volume-storage-location}
...
### [ボリュームの特徴](#volume-characteristics) {#volume-characteristics}

{: .labeled}
| 特徴 | 説明 |
| -------------------- | ------------------------------------------------------- |
| Docker が管理 | Docker が保存場所を管理する<br>ユーザーはパスを意識しない |
...
### [バインドマウントの使用](#bind-mount-usage) {#bind-mount-usage}
...
### [バインドマウントの特徴](#bind-mount-characteristics) {#bind-mount-characteristics}
...
### [ボリュームとバインドマウントの違い](#volume-vs-bind-mount) {#volume-vs-bind-mount}
...
## [tmpfs マウント](#tmpfs-mount) {#tmpfs-mount}

<strong>tmpfs マウント</strong>は、ホストのメモリ上に一時的なファイルシステムを作成し、コンテナにマウントする仕組みです

### [tmpfs マウントの使用](#tmpfs-mount-usage) {#tmpfs-mount-usage}
...
### [tmpfs マウントの特徴](#tmpfs-mount-characteristics) {#tmpfs-mount-characteristics}
...
## [ボリューム・バインドマウント・tmpfs の比較](#storage-options-comparison) {#storage-options-comparison}
...
### [どれを使うべきか](#which-to-use) {#which-to-use}
...
## [ストレージドライバ](#storage-driver) {#storage-driver}

<strong>ストレージドライバ</strong>は、コンテナのレイヤ構造と Copy-on-Write を実現するカーネルレベルの仕組みです

### [ストレージドライバの役割](#storage-driver-role) {#storage-driver-role}

ストレージドライバは、イメージレイヤとコンテナレイヤを管理し、コンテナから見える統合されたファイルシステムを提供します

[03-image](../03-image/) で学んだ overlay filesystem は、ストレージドライバの1つです

### [主なストレージドライバ](#main-storage-drivers) {#main-storage-drivers}

{: .labeled}
| ドライバ | カーネル機能 | 特徴 |
| -------------- | ---------------------- | ------------------------------------------------------------- |
| overlay2 | overlayfs | 現在の標準<br>ほとんどの Linux ディストリビューションでサポート |
| fuse-overlayfs | FUSE + overlayfs | rootless コンテナ向け<br>ユーザー権限で動作可能 |
| btrfs | Btrfs ファイルシステム | Btrfs のスナップショット機能を利用 |
| zfs | ZFS ファイルシステム | ZFS のスナップショット機能を利用 |

### [overlay2 の動作](#overlay2-operation) {#overlay2-operation}

overlay2 は [03-image](../03-image/) で学んだ overlayfs を使います
...
### [ボリュームとストレージドライバの関係](#volume-and-storage-driver-relationship) {#volume-and-storage-driver-relationship}
...
次のトピック [06-security](../06-security/) では、<strong>コンテナのセキュリティ</strong>を学びます
...
| 用語 | 説明 |
| ---------------------- | ------------------------------------------------------------------------------------- |
| コンテナレイヤ | コンテナ起動時にイメージの上に追加される書き込み可能なレイヤ<br>コンテナ削除で消える |
| 一時性（ephemeral） | コンテナのファイルシステムがコンテナのライフサイクルと結びついている性質 |
| ボリューム | Docker が管理する永続ストレージ<br>コンテナのライフサイクルとは独立してデータを保持する |
...
| overlay2 | 現在標準のストレージドライバ<br>overlayfs を使ってレイヤ構造を実現する |
| fuse-overlayfs | rootless コンテナ向けのストレージドライバ<br>FUSE を使ってユーザー権限で動作する |
...
- [tmpfs(5) - Linux manual page](https://man7.org/linux/man-pages/man5/tmpfs.5.html){:target="\_blank"}
  - メモリベースのファイルシステム

<strong>Docker ストレージ</strong>

- [Docker storage overview](https://docs.docker.com/engine/storage/){:target="\_blank"}
