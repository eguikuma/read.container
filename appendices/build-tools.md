...
# [appendix：ビルドツール](#build-tools) {#build-tools}

## [はじめに](#introduction) {#introduction}

[03-image](../../03-image/) では、Dockerfile と `docker build` でコンテナイメージをビルドする仕組みを学びました

[alternative-runtimes](../alternative-runtimes/) では、コンテナランタイムに Docker 以外の選択肢があることを学びました
...
## [ビルドツールの分類](#build-tool-classification) {#build-tool-classification}
...
## [classic builder と BuildKit](#classic-builder-and-buildkit) {#classic-builder-and-buildkit}
...
### [classic builder の動作](#classic-builder-behavior) {#classic-builder-behavior}
...
### [BuildKit の動作](#buildkit-behavior) {#buildkit-behavior}
...
マルチステージビルド（[multi-stage-build](../multi-stage-build/)）では、独立したステージが並列にビルドされます

### [BuildKit の主な改善点](#buildkit-improvements) {#buildkit-improvements}
...
### [キャッシュマウント](#cache-mount) {#cache-mount}
...
### [シークレットマウント](#secret-mount) {#secret-mount}
...
[alternative-runtimes](../alternative-runtimes/) で学んだ Podman が「コンテナの実行」を担うのに対し、Buildah は「イメージのビルド」を担います

### [特徴](#buildah-features) {#buildah-features}
...
### [Dockerfile を使わないビルド](#build-without-dockerfile) {#build-without-dockerfile}
...
### [なぜ Kaniko が必要か](#why-kaniko) {#why-kaniko}
...
### [特徴](#kaniko-features) {#kaniko-features}
...
## [ビルドツールの選択指針](#build-tool-selection-guide) {#build-tool-selection-guide}
...
- [BuildKit - GitHub](https://github.com/moby/buildkit){:target="\_blank"}
  - Docker の標準ビルドエンジン
- [Docker Build architecture](https://docs.docker.com/build/architecture/){:target="\_blank"}
  - BuildKit のアーキテクチャと機能

<strong>Buildah</strong>

- [Buildah - GitHub](https://github.com/containers/buildah){:target="\_blank"}
  - Podman エコシステムのビルドツール
- [Buildah tutorials](https://github.com/containers/buildah/tree/main/docs/tutorials){:target="\_blank"}
  - Buildah の使い方のチュートリアル

<strong>Kaniko</strong>

- [Kaniko - GitHub（Google オリジナル・アーカイブ済み）](https://github.com/GoogleContainerTools/kaniko){:target="\_blank"}
  - Kubernetes 環境向けのコンテナイメージビルドツール
- [Kaniko - GitHub（Chainguard フォーク）](https://github.com/chainguard-dev/kaniko){:target="\_blank"}
